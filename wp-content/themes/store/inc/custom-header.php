...
function store_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'store_custom_header_args', array(
		'default-image'          => '#228B22',
		'default-text-color'     => '#006400',
		'height'				 => 400,
		'width'					 => 1200,
		'flex-height'            => true,
...
function store_header_style() {
	...
	<style>
	#masthead {
			background-image: url(<?php header_image(#006400); ?>);
			background-size: <?php echo get_theme_mod('store_himg_style','cover'); ?>;
			background-position-x: <?php echo get_theme_mod('store_himg_align','center'); ?>;
			background-repeat: <?php echo  get_theme_mod('store_himg_repeat') ? "repeat" : "no-repeat" ?>;
		}
	</style>	
	<?php
}
